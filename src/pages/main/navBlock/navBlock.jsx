...
function NavBlock (){
  ...
  return(
     <div  className="NavBlock">
       ...
       <Link to="/" className={`linkToHome no-select ${loaded ? "comeFromUp2" : ""}`} ><DubbleLetter text={"Fytechnology LTD"} trns={0.08} state={loaded}/></Link>
       
       <div className="buttonsBlock no-select">
        
         <button className={`menuBtn ${loaded ? "comeFromDown2" : ""}`}>
           <div className="line"></div>
           <div className="line"></div>
         </button>
         
         
         <Link onMouseOver={()=>setBtn1(true)} onMouseLeave={()=>setBtn1(false)} to="/contact" className={`contactBtn ${loaded ? "comeFromRight2" : ""}`}>
